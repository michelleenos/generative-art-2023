...
new p5(
    (p: p5) => {
        ...
        function setupPane() {
            ...
            // f.addBinding(C, 'showGravity', { label: 'show gravity' })
            // f.addBinding(C, 'showCells', { label: 'show cells' })
            // f.addBinding(C, 'symmetry', { options: ['reflect', 'rotate'] })
            f.addBinding(C, 'symmetry', {
                options: { reflect: 'reflect', rotate: 'rotate', none: 'none' },
            }).on('change', (e) =>
                lines.forEach((l) => (l.symmetry = e.value as 'reflect' | 'rotate' | 'none')),
            )

            f.addBinding(lines[0], 'color', { label: 'line1' })
            f.addBinding(lines[1], 'color', { label: 'line2' })
            f.addBinding(lines[2], 'color', { label: 'line3' })

            // f.addBinding(C, 'blendMode', {
            //     options: {
            //         multiply: p.MULTIPLY,
            //         normal: p.BLEND,
            //         screen: p.SCREEN,
