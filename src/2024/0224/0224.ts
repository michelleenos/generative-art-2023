...
new p5(
    (p: p5) => {
        ...
        fold.addBinding(C, 'symmetry', {
            options: { rotation: 'rotation', reflection: 'reflection' },
        })
        fold.addBinding(C, 'durAlpha', { min: 0, max: 5 })
        fold.addBinding(C, 'durSize', { min: 0, max: 5 })
        fold.addBinding(C, 'durWait', { min: 0, max: 5 })
        fold.addBinding(C, 'stepRate', { min: 1, max: 60, step: 1 })
        fold.addBinding(C, 'walk', { options: { weighted: 'weighted', random: 'random' } })
        fold.addBinding(C, 'showBase')
        ...
        fold.addBinding(C, 'sizeMax', { min: 0, max: 5, step: 0.001 }).on('change', checkSizes)
        fold.addBinding(C, 'sizeStep', { min: 0, max: 2, step: 0.001 }).on('change', checkSizes)
        fold.addBinding(C, 'count', { min: 5, max: 50, step: 1 }).on('change', doSetup)
            ...
            for (let x = 0; x < count; x++) {
                for (let y = 0; y < count; y++) {
                    ...
                    if (C.symmetry === 'reflection') {
                        ...
                        p.circle(
                            cells[oppx][oppy].posx,
                            cells[oppx][oppy].posy,
                            size * cell.amountSize,
                        )
                    } else {
                        p.circle(cell.posx, cell.posy, size * cell.amountSize)
