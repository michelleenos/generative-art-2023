...
const setPane = (onChangeSize: () => void) => {
    ...
    let stepSizeInput = folder.addBinding(props, 'stepSize', { min: 10, max: 200, step: 1 })
    stepSizeInput.on('change', () => onChangeSize())

    folder.addBinding(props, 'speed', { min: 0, max: 0.001, step: 0.000001 })
    folder.addBinding(props, 'speedCells', { min: 0, max: 0.001, step: 0.000001 })
    folder.addBinding(props, 'varyX', { min: -2, max: 2, step: 0.05 })
    folder.addBinding(props, 'varyY', { min: -2, max: 2, step: 0.05 })
    folder.addBinding(props, 'varyX2', { min: -2, max: 2, step: 0.001 })
    folder.addBinding(props, 'varyY2', { min: -2, max: 2, step: 0.001 })
    folder.addBinding(props, 'progressFnVary', {
        view: 'list',
